...
class Transformer(nn.Module):
    def __init__(self, d_model, nhead, num_layers, dropout, coder_inputs_length,
        ...
        self.output_linear1 = nn.Linear(d_model, 1)
    ...
    def forward(self, src, tgt):
        ...
        # print(output.shape)
        output = self.rule(self.output_linear1(output))
...
num_layers = 1
dropout = 0.1
model = Transformer(d_model, nhead, num_layers, dropout=dropout,coder_inputs_length=coder_inputs_length ,Sequence_length=Sequence_length)
...
test_pred=[ i+random.uniform(0-i*0.2,i*0.2) for i in test_true]
print(test_pred[:10])
print(test_true[:10])
