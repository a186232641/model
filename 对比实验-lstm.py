...
class ACmix(nn.Module):
    def __init__(self, feature_size=64, num_layers=2, dropout=0.1):  # 这个feature_size=200的维度要与输入transformer中的每个单元的维度是一样的
        ...
        self.lstm1 = nn.LSTM(self.hidden_size, self.hidden_size*2, num_layers=1)  # RNN

        self.linear_1 = nn.Linear(16, 1)
    ...
    def forward(self, src):
        ...
        src,_=self.lstm1(src)

        attn_output = self.linear_2(self.relu(self.linear_1(src).squeeze(2)))  # .squeeze(2)
